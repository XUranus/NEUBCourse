...
class PrintField extends React.Component {
  ...
  render() {
    const {data,patentMedicinePrescriptionList,herbalMedicinePrescriptionList} = this.props;
    ...
    return (
      <Card style={{margin:20,width:'900px'}}>
        ...
        <Row>
          <Col span={4}><b>处方：</b></Col>
          <Col span={20}>
            {herbalMedicinePrescriptionList.map(prescription=>(
              <div key={prescription.id}>
                {prescription.status==="已提交"?prescription.prescription_item_list.map(x=>(
                  <Typography.Paragraph key={x.id}>
                    <span style={{marginRight:'20px'}}>{x.drug.code}</span>
                    <span style={{marginRight:'20px'}}>{x.drug.name}</span>
                    <span style={{marginRight:'20px'}}>用量：{x.usage}</span>
                    <span style={{marginRight:'20px'}}>数量:{x.amount}</span>
                    <span style={{marginRight:'20px'}}>医嘱：{x.note}</span>
                  </Typography.Paragraph>
                )):null}
              </div>
            ))}
...
class MedicalRecordPrint extends React.Component {
  render() {
    const {patentMedicinePrescriptionList,data,herbalMedicinePrescriptionList} = this.props;
    return (
