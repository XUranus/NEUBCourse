...
function render_li(day){
	str = 
	 `<li onclick=changeTheme('${day.type}')><div>${day.week}</div><img src="img/${characterToType(day.type)}" alt="Rain"><br> <b>${day.high.split(' ')[1].split('.')[0]}</b> / ${day.low.split(' ')[1].split('.')[0]}</li>`;
	 return str;
 }

 function render_forecast(days) {
	 str = '<ul>';
	 for(var i=0;i<days.length;i++)
	   str += render_li(days[i])+'&nbsp;&nbsp;&nbsp;&nbsp;';
	 str += '</ul>';
	 $('#forecast').html(str);
 }
...
function getCityNodeByName(name){
	for(var i=0;i<cities.length;i++) {
		if(cities[i].city_name==name) {
			if(cities[i].city_code=="") return null;
			else return cities[i].city_code;
		}
	}
	return null;
}

function getCityNameByNode(node){
	for(var i=0;i<cities.length;i++) {
		if(cities[i].city_code==node) {
...
function setCityName(){
	var name=prompt("输入你的城市名","")
	if (name!=null && name!="") {
		var city_id = getCityNodeByName(name);
		if(city_id!=null) {
			setCookie('id',city_id);
			location.reload();
		} else setCityName();
	}
}
...
function changeTheme(type) {
 ...
 window.onload = function(){
	 var city_id = getCookie('id');
	 if(city_id==null){
		setCityName();
	 } else {
		 $('#cityName').html(getCityNameByNode(city_id));
	 }

	 $.get("http://t.weather.sojson.com/api/weather/city/"+city_id,function(res,status){
		 if(status!='success') {
			 alert('错误!');
		 } else {
			 today = res.data.forecast[0];
			 console.log(today);
			 $('#temperature').html(`${res.data.wendu}`);
			 $('#right-information').html(`<span>pm2.5: ${res.data.pm25}</span>&nbsp;&nbsp;&nbsp;&nbsp; <span>湿度: ${res.data.shidu}</span><br> <span>${res.data.ganmao}</span>`);
			 $('#time').html(today.ymd + '&nbsp;&nbsp '+today.week);
			 $('#weatherState').html(`${today.type} ,&nbsp;<span> ${today.notice}</span>`);

			 var forecast = [];
			 forecast.push(res.data.yesterday);
			 for(var i=0;i<res.data.forecast.length;i++)
				 forecast.push(res.data.forecast[i]);
				 //console.log(forecast);
			 render_forecast(forecast);
			 
			 $('#todayImg').attr('src','img/'+characterToType(today.type));
			 changeTheme(today.type);
		 }
	 });
 }

 function characterToType(ch) {
	 if(ch=='阴' || ch=='多云') {
		 return 'cloudy.png';
	 } else if(ch=='晴') {
		 return 'sunny.png';
