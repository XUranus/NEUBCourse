...
function refresh() {
  $.ajax({//向服务器提交数据
    ...
    success:function(data) {
      ...
      console.log('my share list',data);
      renderFileList(data);
    },
...
function renderFileList(data) {//data 是一个含有两个数组的json
    ...
    if(result=='') $('#newTable').append('<tr><td>你还没有分享文件哦...QwQ</td></tr>');
}
...
function cancelShare(file_id) {
  $.ajax({//请求token
		url:CANVEL_SHARE_URL,
		data:{file_id:file_id},
		type:'post',
		async:false,
		dataType:"json",
